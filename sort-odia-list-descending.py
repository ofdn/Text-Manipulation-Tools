# This Python tool sorts a list of text lines based on the years specified in Odia script, ensuring that the list is arranged in descending order from the latest to the earliest year. It outputs the sorted list to a new file, preserving the original content.
...
def main(filename):
    ...
    # Sort lines by year in descending order; non-matching lines will be None and go to the end
    sorted_lines = sorted(lines_with_years, key=lambda x: (x[1] is None, x[1]), reverse=True)
